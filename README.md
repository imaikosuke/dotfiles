...
現状の構成は Apple Silicon 向けの 1 台構成で、`flake.nix` の `username`、`hostname`、`system` を起点に各モジュールへ値を渡しています。
...
- `flake.nix`
  - flake の入力と出力を定義します。
  - `username`、`hostname`、`system` をまとめて定義し、`darwinConfigurations.${hostname}` がこの環境のエントリーポイントです。
- `nix-darwin/configuration.nix`
  - macOS 側の基本設定をまとめます。
  - `flake.nix` から受け取った `system`、`username` を使って、プラットフォーム、ユーザーのホームディレクトリ、Touch ID sudo などを定義します。
- `nix-darwin/home_manager.nix`
  - `Home Manager` を `nix-darwin` に統合します。
  - `flake.nix` から受け取った `username` を Home Manager 側へ渡し、`home-manager/home.nix` を読み込みます。
- `nix-darwin/homebrew.nix`
  - `nix-homebrew` と Homebrew 管理の設定です。
  - `username` を使って Homebrew の所有ユーザーを定義し、`brew` と `cask` の一覧、更新ポリシーを管理します。
- `home-manager/home.nix`
  - ユーザー単位の設定です。
  - `home.packages`、`home.file`、`programs` などを管理します。
...
- Home Manager
  ...
  - `fzf`（`programs.fzf` で有効化し、zsh 連携も有効）
  - `starship`（`programs.starship` で有効化し、zsh 連携も有効）
  - `sghq`（`fzf` + `ghq` でリポジトリ検索して移動する zsh 関数）
  - Ghostty 上での `cd` 後の自動 `ls`（`chpwd.zsh`）
  - Ghostty 起動時の zsh バナー（`TERM_PROGRAM=ghostty` のインタラクティブシェルで `banner.txt` を表示）
...
- `home-manager/home.nix`
  - `home.file` で `home-manager/zsh/` ディレクトリ全体を `~/.config/zsh` に配置します。
...
- `home-manager/zsh/.zshrc`
  ...
  - Ghostty 上では `TERM_PROGRAM=ghostty` のときだけ、`~/.config/ghostty/banner.txt` を Tokyo Night の青系アクセント色で表示します。
...
現在は `flake.nix` の `username = "imai";` を起点に、各モジュールでユーザー名とホームディレクトリを組み立てています。

別ユーザーで使う場合は、まず `flake.nix` の `username` を変更してください。

### アーキテクチャ

`flake.nix` では次を指定しています。

```nix
system = "aarch64-darwin";
```

この値が `nix-darwin/configuration.nix` の `nixpkgs.hostPlatform` に渡されます。Intel Mac で利用する場合は、環境に合わせて見直しが必要です。
